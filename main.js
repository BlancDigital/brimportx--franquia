  ...
  entries.forEach((entry) => {
    if (entry.isIntersecting && entry.intersectionRatio < 0.6) {
      ...
      for (i; i <= 2; i++) {
        // from-left animation
        if (afl.includes(i)) {
          entry.target.children[i].style.animation = "afl 1s both"
        }

        // from-right animation
        if (afr.includes(i)) {
          entry.target.children[i].style.animation = "afr 1s both"
        }

        // top-to-bottom animation
        if (aft.includes(i)) {
          entry.target.children[i].style.animation = "aft 1s both"
        }

        // bottom-to-top animation
        if (afb.includes(i)) {
          entry.target.children[i].style.animation = "afb 1s both"
        }
      }
    }
  })
